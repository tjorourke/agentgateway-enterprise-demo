...
step "Deleting catalog entries"
for a in $AGENTS; do arctl delete agent "$a" >/dev/null 2>&1 && ok "agent $a" || log "no agent $a"; done
arctl delete mcp acme/textkit  >/dev/null 2>&1 && ok "mcp acme/textkit" || true
arctl delete skill summary-style >/dev/null 2>&1 && ok "skill summary-style" || true
